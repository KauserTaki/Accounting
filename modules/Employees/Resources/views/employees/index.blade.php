            ...
            <table class="table table-flush table-hover">
                <thead class="thead-light">
                    <tr class="row table-head-line">
                        ...
                        <th class="col-md-2 hidden-xs">@sortablelink('contact.user_id',trans('Official ID'))</th>
                        <th class="col-md-3">@sortablelink('contact.name', trans('general.name'))</th>
                        <th class="col-md-2 hidden-xs">@sortablelink('contact.email',trans('general.email'))</th>
                ...
                <tbody>
                    @foreach($employees as $employee)
                        <tr class="row align-items-center border-top-1">
                            ...
                            <td class="col-md-2 border-0 hidden-xs">{{ !empty($employee->contact) ? $employee->company->name[0] : '-' }}{{$employee->position->name[0]}}{{$employee->contact->name[0]}}{{str_pad($employee->contact->id, 5, '0', STR_PAD_LEFT)}} </td>
                            <td class="col-md-3 border-0"><a href="{{ route('employees.employees.show', $employee->id) }}">{{ !empty($employee->contact) ? $employee->contact->name : '-' }}</a></td>
                            <td class="col-md-2 border-0 hidden-xs">{{ !empty($employee->contact) ? $employee->contact->email : '-' }}</td>
